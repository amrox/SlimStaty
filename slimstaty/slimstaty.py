...
class StateMachine(object):
    ...
    class Transition(object):
        ...
        def __repr__(self):
            return f"Transition({self._event}: {self._from_state} -> {self._to_state})"
    ...
    def transitions(self):
        return [self.Transition(x['name'], x['from'], x['to']) for x in self._transition_table]
    ...
    def from_yaml(cls, path: str) -> StateMachine:
        with open(path, 'r') as stream:
            try:
                ...
                return cls(y['statemachine']['name'], y['statemachine']['events'])
